...
#define EI_CLASSIFIER_PROJECT_ID                 650178
#define EI_CLASSIFIER_PROJECT_OWNER              "Industrial Datasets Inc."
#define EI_CLASSIFIER_PROJECT_NAME               "Booth Demo: Qualcomm Dragonwing RB3 Gen 2 Car Detection - V2"
...
#define EI_CLASSIFIER_NN_OUTPUT_COUNT            3200
...
#define EI_CLASSIFIER_OBJECT_TRACKING_ENABLED    1
...
typedef struct {
    int id;
    int last_ground_truth_update_t;
    const char *label;
    uint32_t x;
    uint32_t y;
    uint32_t width;
    uint32_t height;
    float value;
    std::tuple<int, int, int, int> last_centroid_segment;
} ei_object_tracking_trace_t;

typedef struct {
    ei_object_tracking_trace_t *open_traces;
    uint32_t open_traces_count;
} ei_object_tracking_output_t;

typedef struct {
    ei_object_tracking_output_t object_tracking_output;
