...
class DataRepository(private val context: Context, private val apiKeyStore: ApiKeyStore) {
    ...
    fun appendWearSamples(sensorKey: String, timestampMs: Long, values: FloatArray) {
        if (isLoggingOffline) {
            try {
                // Prepend key to headers so it's readable in the unified CSV
                val data = values.mapIndexed { i, v -> "${sensorKey}_$i" to v }.toMap()
                saveSensorData(SensorData(timestampMs, data))
            } catch (e: Exception) {
                Log.e("DataRepository", "Wear CSV log error", e)
            }
        }
        if (!isMultiRecording) return
